...
bool is_avl_tree(bag_t *bag){
    // return True if the BST is a AVL
...
bool is_avl_node(avl_node_t *root){
    // return True if the BST is a AVL
    int ll = (int)(HEIGHT(root->left));
    int lr = (int)(HEIGHT(root->right));
    int bf = lr - ll;
...
// INSERTING BST
// Helper Function
bool avl_insert_norot(avl_node_t **root, bag_elem_t elem, int (*cmp)(bag_elem_t, bag_elem_t))
{
    bool inserted;
...
bool bag_insert_norot(bag_t *bag, bag_elem_t elem) {
    return avl_insert_norot(&(bag->root), elem, bag->cmp);
}
...
bool my_avl_remove(avl_node_t **root, bag_elem_t elem, int (*cmp)(bag_elem_t, bag_elem_t)){
    ...
    if (! *root)
        ...
        removed = my_avl_remove(&(*root)->left, elem, cmp);
        if (removed) {
            if (HEIGHT((*root)->left) + 1 < HEIGHT((*root)->right)) // left too small
                    avl_rebalance_to_the_left(root);
                else
                    avl_update_height(*root);
        }
        ...
        removed = my_avl_remove(&(*root)->left, elem, cmp);
        if (removed) {
            if (HEIGHT((*root)->right) + 1 < HEIGHT((*root)->left)) // right too small
                    avl_rebalance_to_the_right(root);
                else
                    avl_update_height(*root);
        }    
